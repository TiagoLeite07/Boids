...
VSFontLib::load(std::string fontName) 
{
	...
	fp = fopen(s.c_str(),"r");
	if (fp == NULL) {
		VSResourceLib::mLogError.addMessage("Unable to find font texture: %s", s.c_str());
		return false;
	}
	...
	if (!loadOK) {
		VSResourceLib::mLogError.addMessage("Problem reading the XML font definition file: %s", s.c_str());
		return false;
	}
	...
	VSResourceLib::mLogInfo.addMessage("Font has %d chars", numChars);
	return true;
}
